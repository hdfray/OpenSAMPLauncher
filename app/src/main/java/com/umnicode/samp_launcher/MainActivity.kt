...
class PermissionRequest(
    ...
    var Callbacks: MutableList<PermissionRequestCallback>
) {}
...
class MainActivity : AppCompatActivity() {
    ...
    fun RequestPermission(Permission: String, CallbackResult: PermissionRequestCallback) {
        ...
        if (PermissionReq != null) { // If permission request exist - add current callback to list
            ...
            this.PermissionRequests.add(
                PermissionRequest(
                    Permission,
                    mutableListOf(CallbackResult)
                )
            ) // Create new request
                            ...
                            object : PermissionRequestCallback {
                                override fun Finished(IsGrantedR: Boolean) {
                                    ...
                                    if (IsGrantedR) {
                                        Log.i(
                                            "PermissionsChecker",
                                            "Storage read permission is granted"
                                        )
                                    } else {
                                        Log.i(
                                            "PermissionsChecker",
                                            "Storage read permission is denied"
                                        )
                                    }
                                    ...
                                    if (IsGrantedR) {
                                        Log.i(
                                            "PermissionsChecker",
                                            "Storage write permission is granted"
                                        )
                                    } else {
                                        Log.i(
                                            "PermissionsChecker",
                                            "Storage write permission is denied"
                                        )
                                    }
